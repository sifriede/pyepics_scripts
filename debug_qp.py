...
def fitparabel(fitfkt, kl, sigx**2/1e6, sigxerr**2/1e6, par, seite):
    """sigx**2/1e6 = sig in m, sigxerr**2/1e6 = sigsqma_sigxerr**2/1e6 in m, ifixb = variiere, wenn Parameter = 1, sonst 0"""
    # Relativistic parameter
    gamma = 1.196
    beta = 0.5482
    
    if len(kl) != len(sigx**2/1e6):
        print("Number of k-values does not match number of sigma-squared valus.")
        print("Aborting")
        return None
        
    par = [startpar, ifixb]
    fkt = odr.Model(fitfkt1)
    data = odr.Data(kl, sigx**2, we=1. / np.power(sigxerr**2, 2))
